...
public class ActivityServiceImpl implements ActivityService {
  ...
  public void createActivity(Session session, Content targetLocation, ActivityServiceCallback callback) throws AccessDeniedException, StorageClientException, ServletException, IOException {
    ...
    Resource target = callback.processRequest(activtyNode);


    activtyNode = contentManager.get(activtyPath);
    activtyNode.setProperty(PARAM_ACTOR_ID, session.getUserId());
    contentManager.update(activtyNode);
    // post the asynchronous OSGi event
    final Dictionary<String, String> properties = new Hashtable<String, String>();
    properties.put(UserConstants.EVENT_PROP_USERID, session.getUserId());
