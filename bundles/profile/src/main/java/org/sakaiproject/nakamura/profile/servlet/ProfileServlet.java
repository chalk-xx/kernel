...
@ServiceDocumentation(bindings={
    @ServiceBinding(type=BindingType.TYPE,
        bindings={UserConstants.GROUP_PROFILE_RESOURCE_TYPE, UserConstants.USER_PROFILE_RESOURCE_TYPE},
        extensions={@ServiceExtension(name="json",description="json format")},
        selectors={@ServiceSelector(name="profile", description="Binds to the selector profile")})},
    methods={
      @ServiceMethod(name="GET",
          description="Responds to simple GET method requests",
          response={
             @ServiceResponse(code=200,
                 description="Responds with a 200 if the request was sucessfull, the output is a json " +
                 		"tree of the profile with external references expanded."),
             @ServiceResponse(code=404,
                 description="Responds with a 404 is the profile node cant be found, body contains no output"),
             @ServiceResponse(code=403,
                 description="Responds with a 403 if the user does not have permission to access the profile or part of it"),
             @ServiceResponse(code=500,
                 description="Responds with a 500 on any other error")}
      )
     },
     name="Profile Servlet",
     description={
...
@SlingServlet(extensions={"json"}, methods={"GET"}, resourceTypes={UserConstants.GROUP_PROFILE_RESOURCE_TYPE, UserConstants.USER_PROFILE_RESOURCE_TYPE}, selectors={"profile"})
public class ProfileServlet extends SlingSafeMethodsServlet {
  ...
  protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response)
    ...
    try {
      profileService.writeProfileMap(resourceNode, response.getWriter());
    } catch ( AccessDeniedException e ) {
      LOGGER.warn("Failed to access profile at {}: {}",new Object[] {resource.getPath(),e.getMessage(),e});
      response.reset();
