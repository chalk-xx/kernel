...
public class MostActiveContentSearchBatchResultProcessorTest {
    ...
    private Logger logger = LoggerFactory.getLogger(MostActiveContentSearchBatchResultProcessorTest.class);
    private SlingHttpServletRequest request = mock(SlingHttpServletRequest.class, RETURNS_DEEP_STUBS);
    ...
    public void testWritingStopsAfterTimeWindow() throws Exception {
        ...
        assertEquals("{\"content\":[{\"id\":\"nodeId#1\",\"name\":\"my-term-paper.pdf\",\"count\":1},{\"id\":\"nodeId#2\",\"name\":\"happy-clam.png\",\"count\":1}]}",
                stringWriter.toString());

    }

    @Test
    public void testExceptionDoesntStopTheFeed() throws Exception {
        Calendar today = Calendar.getInstance();

        Node badNode = prepareAnActivityNode("badNode", today);
        when(badNode.getProperty(anyString())).thenThrow(new RepositoryException("I am a bad bad node!!"));
        ...
        assertEquals("{\"content\":[{\"id\":\"goodNode#1\",\"name\":\"a-good-joke.txt\",\"count\":1}]}", stringWriter.toString());
    }
    ...
    public void testCanCountProperly() throws Exception {
        ...
        assertEquals("{\"content\":[{\"id\":\"node#1\",\"name\":\"fastest-cars.doc\",\"count\":2}]}", stringWriter.toString());

    }

    private void loadSession(Map<String, Node> activityNodes, Map<String, Node> resourceNodes) throws RepositoryException, NoSuchAlgorithmException, UnsupportedEncodingException {
        ...
        for (String id : resourceNodes.keySet()) {
            when(session.getNode(CreateContentPoolServlet.hash(id))).thenReturn(resourceNodes.get(id));
        }
    }
    ...
    private Node prepareAResourceNode(String resourceName) throws RepositoryException {
        ...
        when(resourceNode.getProperty("sakai:pooled-content-file-name").getString()).thenReturn(resourceName);
        return resourceNode;
    }
}
