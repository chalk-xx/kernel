...
public class ModifyOperation extends AbstractCreateOperation {
    ...
    @Override
    protected void doRun(SlingHttpServletRequest request, HtmlResponse response, ContentManager contentManager, List<Modification> changes)
        ...
        Content content = processCreate(contentManager, reqProperties, response, changes, versioningConfiguration);

        // write content from existing content (@Move/CopyFrom parameters)
        processMoves(content, contentManager, reqProperties, changes, versioningConfiguration);
