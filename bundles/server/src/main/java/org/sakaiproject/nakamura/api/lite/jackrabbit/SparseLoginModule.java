...
public class SparseLoginModule extends AbstractLoginModule {
	...
	public void initialize(Subject subject, CallbackHandler callbackHandler,
		...
		CallbackHandlerWrapper wrappedCallbackHandler = new CallbackHandlerWrapper(
	...
	protected boolean impersonate(Principal principal, Credentials credentials)
		...
		if (user != null) {
			...
			if (!impersSubject.getPrincipals(AdminPrincipal.class).isEmpty()
					|| !impersSubject.getPrincipals(SystemPrincipal.class)
							.isEmpty()) {
				return true;
			}
			...
			throw new FailedLoginException("attempt by user "
					+ principal.getName() + " with subjects "
					+ impersSubject.getPrincipals() + " to impersonate "
					+ credentials);
		}
		return false;
	}
	...
	protected Principal getPrincipal(Credentials credentials) {
		...
		if (auser != null) {
			...
			return new SparsePrincipal(userId, this.getClass().getName()
					+ " credentials were "
					+ ((credentials == null) ? null : credentials.getClass()
							.getName()), SparseMapUserManager.USERS_PATH);
		} else {
			LOGGER.debug(
					"No User Found in credentials  UserID[{}] Credentials[{}]",
					userId, credentials);
		}
		return null;
	}
