...
=head2 OSGi (Apache Felix)

Whilst Sling is not an OSGi (http://www.osgi.org/About/WhatIsOSGi) framework
implementation specific application, it features an embedded Apache Felix OSGi
framework and console that provides a dynamic runtime environment, where code
and content bundles can be loaded, unloaded and reconfigured at runtime. It is
expected that Sling will also operate inside other OSGi frameworks such as
