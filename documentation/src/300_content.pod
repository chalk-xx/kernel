...
the resourceType are considered. The resource handler decides how output is
delivered. In the example above, a POST to /content/mynode would resolve to
/apps/foo/bar/POST.esp, with a similar mapping for other methods.
