...
class TC_MySiteTest < SlingTest
  ...
  def test_add_group_to_site
   ...
   test_site = create_site("somesite"+@m)
   test_site.add_group(site_group.name)
   groups = SlingSites::Site.get_groups("somesite"+@m, @s)
  ...
  def test_join_unjoinable_site
    ...
    test_site = create_site("someothersite"+@m)
    test_site.add_group(site_group.name)
  ...
  def test_join_and_search
    ...
    res = @s.update_node_props("anothersite"+@m, "fish" => "dog")
    ...
    created_site = result["results"].select { |r| r["path"] == "/anothersite"+@m }
