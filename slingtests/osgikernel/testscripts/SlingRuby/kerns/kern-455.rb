...
class TC_Kern455Test < SlingTest
  ...
  def test_save_uploaded_file
	...
	assert_equal((res.body.include?"created(\"/_user/d0/33/e2/2a/admin/private/testnode#{m}\""), true, "Expected to find hashed created path "+res.body)
	assert_equal((res.body.include?"modified(\"/_user/d0/33/e2/2a/admin/private/testnode#{m}/testprop\""), true, "Expected to find the hashed created path "+res.body)
	...
	res = @s.execute_get(@s.url_for("_user/d0/33/e2/2a/admin/private/testnode#{m}.json"))
	...
	assert_equal((res.body.include?"created(\"/_user/d0/33/e2/2a/admin/private/test/n/o/d/e/#{m}\""), true, "Expected to find hashed created path "+res.body)
	assert_equal((res.body.include?"modified(\"/_user/d0/33/e2/2a/admin/private/test/n/o/d/e/#{m}/testprop\""), true, "Expected to find the hashed created path "+res.body)

	res = @s.execute_get(@s.url_for(testnode+".json"))
	assert_equal(res.code.to_i, 200, "Expected to be able to get to the node via the private route "+res.body)
	b1 = res.body

	res = @s.execute_get(@s.url_for("_user/d0/33/e2/2a/admin/private/test/n/o/d/e/#{m}.json"))
	assert_equal(res.code.to_i, 200, "Expected to be able to get to the node via the direct route "+res.body)
	assert_equal(b1, res.body, "Expected direct and personal bodies to be the same ")
	
	
    nodename = "_user/d0/33/e2/2a/admin/private/upload_test_admin#{m}"
