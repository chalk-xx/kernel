...
sub create {
    ...
    my $archive = odfContainer ( "$folder/$name.odt", create => 'text' );
    ...
    $meta->description( "ODT document containing random data for test purposes." );
    ...
    unless ( $odt_zip->read( "$folder/$name.odt" ) == AZ_OK ) {
        die "Problem reading zip file: \"$folder/$name.odt\".";
    }
    $odt_zip->removeMember( "content.xml" );
    my $file_member = $odt_zip->addFile( "$folder/$name.odt.tmp", 'content.xml' );
    $file_member->desiredCompressionMethod( COMPRESSION_DEFLATED );
    unless ( $odt_zip->overwrite() == AZ_OK ) {
        die "write error: \"$folder/$name.odt\".";
    }
    unlink( "$folder/$name.odt.tmp" );
    return -s "$folder/$name.odt";
}
#}}}
