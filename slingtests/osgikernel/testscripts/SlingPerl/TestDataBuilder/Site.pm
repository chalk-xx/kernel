...
sub new {
    ...
    my $site = { BaseDir => $testDataDirectory,
		     Verbose => $verbose,
		     Log     => $log };
    bless( $site, $class );
    return $site;
...
sub generate {
	...
	die "Could not clear existing site data file" unless $success;
    }
    Sling::Print::print_with_lock( "\"site\",\"sakai:joinable\"", $site_data_file );

    for ( my $i = 1 ; $i <= 4000 ; $i++ ) {
        Sling::Print::print_with_lock( "\"testsite$i\",\"yes\"", $site_data_file );
    }
