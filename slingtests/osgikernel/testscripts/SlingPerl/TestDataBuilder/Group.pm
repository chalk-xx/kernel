...
sub new {
    ...
    my $group = { BaseDir => $testDataDirectory,
		     Verbose => $verbose,
		     Log     => $log };
    bless( $group, $class );
    return $group;
...
sub generate {
    my ( $group ) = @_;
    Sling::Print::print_with_lock( "Creating group test data.", $group->{ 'Log' });
	...
	die "Could not clear existing group data file" unless $success;
    }
    ...
    my $group_member_data_file = $group->{ 'BaseDir' } . "/group_member_additions.txt";
    if ( -f $group_member_data_file ) {
        my $success = unlink( $group_member_data_file );
	die "Could not clear existing group member data file" unless $success;
    }
    Sling::Print::print_with_lock( "\"group\",\"user\"", $group_member_data_file );
