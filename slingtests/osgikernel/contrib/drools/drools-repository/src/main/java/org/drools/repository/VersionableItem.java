    ...
    protected Node getPrecedingVersionNode() throws RulesRepositoryException {
        try {
            ...
            if ( this.node.getPrimaryNodeType().getName().equals( "nt:version" ) ) {
                ...
                VersionManager versionManager = this.node.getSession().getWorkspace().getVersionManager();
                versionNode = versionManager.getBaseVersion(this.node.getPath());
            }
...
//        }
    ...
    public boolean isArchived() {
    	try {
    	  if ( this.node.hasProperty(CONTENT_PROPERTY_ARCHIVE_FLAG)) {
