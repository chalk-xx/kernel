...
@Service
public final class TrustedTokenService {
  ...
  @Property( intValue = 1200000, description="The TTL of a cookie based token, in ms")
   ...
   * 
   */
  public TrustedTokenService() throws NoSuchAlgorithmException {
  ...
  void refreshToken(HttpServletResponse response, String value, String userId) {
    ...
    if (parts != null && parts.length == 3) {
      long cookieTime = Long.parseLong(parts[1]);
  ...
  String encodeCookie(String userId) {
    ...
    String cookePayload = expires + ";" + userId;
    String cookieValue = expires + ":" + getCurrentToken() + ":" + userId;
    try {
      return sha1Hash(cookieValue) + ";" + cookePayload;
  ...
  String decodeCookie(String value) {
    ...
    if (parts != null && parts.length == 3) {
      ...
      if (System.currentTimeMillis() < cookieTime) {
        for (String pastToken : getCurrentTokens()) {
          String cookieValue = parts[1] + ":" + pastToken + ":" + parts[2];
          try {
            if (parts[0].equals(sha1Hash(cookieValue))) {
              return parts[2];
            } else {
              LOG.info("Token Invalid " + pastToken);
            }
          } catch (NoSuchAlgorithmException e) {
            LOG.error(e.getMessage());
          } catch (UnsupportedEncodingException e) {
  ...
  private synchronized String getCurrentToken() {
    ...
    return currentTokens[currentToken];
  }
