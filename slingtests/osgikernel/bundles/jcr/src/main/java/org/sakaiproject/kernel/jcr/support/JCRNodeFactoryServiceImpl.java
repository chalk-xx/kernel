...
public class JCRNodeFactoryServiceImpl implements JCRNodeFactoryService {
  ...
  private void populateFolder(Node node) throws RepositoryException {
    // JCR Types
    if (debug) {
      LOGGER.debug("Doing populate Folder");
    }
    if (jcrService.needsMixin(node, JCRConstants.MIX_LOCKABLE)) {
      node.addMixin(JCRConstants.MIX_LOCKABLE);
    }
  ...
  private Node createNode(String id, String mimeType, String type)
    ...
    try {
      ...
      if (debug) {
        LOGGER.debug("RelPath is " + relPath + " split into " + pathElements.length
            + " elements ");
        for (String pathel : pathElements) {
          LOGGER.debug("       Path Element is [" + pathel + "]");
        }
      }
      ...
      node = currentNode;
      if (node == null) {
        LOGGER.error("Failed to create Node " + id + " got null ");
        throw new JCRNodeFactoryServiceException("Failed to create node " + id
            + " got null ");
      } else if (!id.equals(node.getPath())) {

        LOGGER.error("Failed to create Node " + id + " got" + node.getPath());
      ...
      LOGGER.warn("Unspecified Repository Failiure ", rex);
      LOGGER.error("Unspecified Repository Failiure " + rex.getMessage());
      if (currentNode != null) {

        try {
          LOGGER.info("Current Node was " + currentNode.getPath() + " saving " + savedNode);
        } catch (RepositoryException e) {
          // TODO Auto-generated catch block
          e.printStackTrace();
        }
      } else {
        LOGGER.info("Current Node was null saving " + savedNode);
      }
    }
    if (node == null) {
      LOGGER.error("NODE IS NULL after Create! ");
    }
    return node;
  ...
  private Node getNodeFromSession(Session s, String id)
    ...
    try {
      ...
      if (debug) {
        LOGGER.debug("getNodeFromSession: Node Does Not Exist :" + id);
      }
      return null;
    }
    Node n = null;
