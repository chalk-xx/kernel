...
public class DelegatedUserAccessControlProvider extends AbstractAccessControlProvider {
    ...
    public void init(Session systemSession, Map configuration) throws RepositoryException {
        ...
        if (systemSession instanceof SessionImpl) {
            ...
            userAdminGroup = (configuration.containsKey(UserAccessControlProvider.USER_ADMIN_GROUP_NAME)) ? configuration.get(UserAccessControlProvider.USER_ADMIN_GROUP_NAME).toString() : UserAccessControlProvider.USER_ADMIN_GROUP_NAME;
            groupAdminGroup = (configuration.containsKey(UserAccessControlProvider.GROUP_ADMIN_GROUP_NAME)) ? configuration.get(UserAccessControlProvider.GROUP_ADMIN_GROUP_NAME).toString() : UserAccessControlProvider.GROUP_ADMIN_GROUP_NAME;
            ...
            usersPath = sImpl.getQPath(UserAccessControlProvider.USERS_PATH);
            groupsPath = sImpl.getQPath(UserAccessControlProvider.GROUPS_PATH);
        } else {
    ...
    private static boolean containsPrincipal(Set principals, String principalName) {
        for (Iterator it = principals.iterator(); it.hasNext() && principalName != null;) {
            Principal p = (Principal) it.next();
    ...
    private class CompiledPermissionsImpl extends AbstractCompiledPermissions
        ...
        protected CompiledPermissionsImpl(Set principals, NodeImpl userNode) throws RepositoryException {
            ...
            isUserAdmin = containsPrincipal(principals, userAdminGroup);
            isGroupAdmin = containsPrincipal(principals, groupAdminGroup);
            

            int events = Event.PROPERTY_CHANGED | Event.PROPERTY_ADDED | Event.PROPERTY_REMOVED;
            observationMgr.addEventListener(this, events, UserAccessControlProvider.USERS_PATH, true, null, null, false);
        }
        ...
        protected Result buildResult(Path path) throws RepositoryException {
          log.info("-------------- Build result for "+path);
            ...
            if (usersPath.equals(abs2Path)) {
                ...
                if (node.isNodeType(UserAccessControlProvider.NT_REP_AUTHORIZABLE)) {
                    authN = node;
                } else if (node.isNodeType(UserAccessControlProvider.NT_REP_AUTHORIZABLE_FOLDER)) {
                    NodeImpl parent = node;
                    while (authN == null && parent.getDepth() > 0) {
                        parent = (NodeImpl) parent.getParent();
                        if (parent.isNodeType(UserAccessControlProvider.NT_REP_AUTHORIZABLE)) {
                            authN = parent;
                        } else if (!parent.isNodeType(UserAccessControlProvider.NT_REP_AUTHORIZABLE_FOLDER)) {
                ...
                if (authN != null && authN.isNodeType(UserAccessControlProvider.NT_REP_USER)) {
                    int relDepth = session.getHierarchyManager().getRelativeDepth(userNode.getNodeId(), authN.getNodeId());
                    boolean isGroupProp = UserAccessControlProvider.P_GROUPS.equals(path.getNameElement().getName());
                ...
                */
                if (isGroupAdmin) {
                  ...
                  // check the group node to see if there are a list of admin groups.
                  Node node = (NodeImpl) getExistingNode(path);
                  if ( node.hasProperty(ADMIN_PRINCIPALS_PROPERTY) ) {
                    Property groups = node.getProperty(ADMIN_PRINCIPALS_PROPERTY);
                    for ( Value group : groups.getValues() ) {
                      String groupName = group.getString();
                      if ( containsPrincipal(userPrincipals, groupName)) {
                        allows = Permission.ALL;
                        if (calcPrivs) {
                            privs |= PrivilegeRegistry.WRITE;
                            log.info("+++++++++++++++++++++User granted permission due to properties");
                        }
                        break;
                      }
                    }
                ...
                log.info("++++++++++++++++++++ Finished checking User ACL");
        ...
        public void onEvent(EventIterator events) {
            while (events.hasNext()) {
                ...
                try {
                    ...
                    String repGroups = session.getJCRName(UserAccessControlProvider.P_GROUPS);
                    // TODO: add better evaluation.
                    
                    // TODO: add groups deliegation invalidation
                    ...
                    if (repGroups.equals(Text.getName(evPath)) &&
                        ...
                        switch (ev.getType()) {
                            ...
                            case Event.PROPERTY_CHANGED:
                                ...
                                String princName = session.getJCRName(UserAccessControlProvider.P_PRINCIPAL_NAME);
