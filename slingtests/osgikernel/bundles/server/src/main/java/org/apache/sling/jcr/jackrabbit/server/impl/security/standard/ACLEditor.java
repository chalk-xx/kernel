...
public class ACLEditor extends DynamicSecurityItemModifier implements
  ...
  ACLEditor(Session editingSession, AccessControlUtils utils) {
    if (editingSession instanceof SessionImpl) {
      ...
      throw new IllegalArgumentException(
          "org.apache.jackrabbit.core.SessionImpl expected. Found "
   ...
   * @param nodePath
   */
  public AccessControlPolicy[] getPolicies(String nodePath)
   ...
   * @param nodePath
   */
  public AccessControlPolicy[] editAccessControlPolicies(String nodePath)
      throws AccessControlException, PathNotFoundException, RepositoryException {
    checkProtectsNode(nodePath);
  ...
  public void setPolicy(String nodePath, AccessControlPolicy policy)
    ...
    NodeImpl aclNode = getAclNode(nodePath);
    /*
     * in order to assert that the parent (ac-controlled node) gets modified an existing
     * ACL node is removed first and the recreated. this also asserts that all ACEs are
   ...
   */
  public synchronized void removePolicy(String nodePath, AccessControlPolicy policy)
  ...
  // --------------------------------------------------------------------------
  /**
   * Check if the Node identified by <code>nodePath</code> is itself part of ACL defining
   * content. It this case setting or modifying an AC-policy is obviously not possible.
   * 
   * @param nodePath
   * @throws AccessControlException
   *           If the given nodePath identifies a Node that represents a ACL or ACE item.
   * @throws RepositoryException
   */
   ...
   * @throws AccessControlException
   */
  private static void checkValidPolicy(String nodePath, AccessControlPolicy policy)
      throws AccessControlException {
    ...
    if (!nodePath.equals(acl.getPath())) {
      throw new AccessControlException("Policy " + policy
          + " cannot be applied/removed from the node at " + nodePath);
    }
  }
   ...
   * 
   * @param node
   *          a name for the child is resolved
   * @param name
   *          if missing the {@link #DEFAULT_ACE_NAME} is taken
   * @return
   * @throws RepositoryException
   */
  protected static Name getUniqueNodeName(Node node, String name)
      throws RepositoryException {
    if (name == null) {
  ...
  /**
   * Build an array of Value from the specified <code>privileges</code> using the given
