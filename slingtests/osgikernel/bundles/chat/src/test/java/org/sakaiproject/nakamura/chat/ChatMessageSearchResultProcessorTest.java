...
public class ChatMessageSearchResultProcessorTest {
	...
	public void aSingleChatResult() throws Exception {
		...
		assertEquals(stringWriter.toString(), "{\"id\":\"-Ab12dc\",\"nationality\":\"USA\",\"state\":\"Texas\"}");
	...
	public void testSelectsSingleToProperty() throws Exception {
		...
		assertEquals("{\"id\":\"-Ab12dc\",\"userTo\":{},\"nationality\":\"USA\",\"state\":\"Texas\"}", stringWriter.toString());
	}
	...
	public void testWeWillWriteTheUserFromProperty() throws Exception {
		...
		// then
		assertEquals("{\"id\":\"-Ab12dc\",\"userFrom\":{},\"nationality\":\"USA\",\"state\":\"Texas\"}", stringWriter.toString());
	}
	...
	private void bareMinimumSetupOfSearchResultProcessorCollaborators() throws RepositoryException {
	...
	private void sessionThatCanReturnChatResultNode() throws PathNotFoundException, RepositoryException {
	...
	private void chatResultWithProperty(String key, String value) throws PathNotFoundException, RepositoryException {
	...
	private void chatResultWithProperty(String key, Boolean value) throws PathNotFoundException, RepositoryException {
		Property newProperty = new MockProperty(key);
		newProperty.setValue(value);
		when(rowNode.hasProperty(key)).thenReturn(Boolean.TRUE);
		when(rowNode.getProperty(key)).thenReturn(newProperty);
	...
	private void chatResultWithSomeCannedProperties() throws RepositoryException {
		when(rowNode.getProperties()).thenReturn(new MockPropertyIterator(mockPropsIterator()));
	}

	private Iterator<Property> mockPropsIterator() throws ValueFormatException, VersionException, LockException, ConstraintViolationException, RepositoryException {
	...
	private void sessionThatCanReturnThisItem(String jcrPath) throws PathNotFoundException, RepositoryException {
