...
public class TagOperationTest {
  ...
  public void testMissingUUID() throws RepositoryException {
    ...
    assertEquals(400, response.getStatusCode());
    assertEquals("Missing uuid parameter", response.getStatusMessage());
  }

  @Test
  public void testProper() throws ItemNotFoundException, RepositoryException {
    SlingHttpServletRequest request = mock(SlingHttpServletRequest.class);
    HtmlResponse response = new HtmlResponse();
    ...
    Node fileNode = new MockNode("/path/to/file");
    when(fileNode.getMixinNodeTypes()).thenReturn(new NodeType[0]);
    
    // Stuff to check if this is a correct request
    when(session.getNodeByUUID(uuid)).thenReturn(tagNode);
