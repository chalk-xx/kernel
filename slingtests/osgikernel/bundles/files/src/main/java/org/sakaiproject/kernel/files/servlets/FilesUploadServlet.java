...
public class FilesUploadServlet extends SlingAllMethodsServlet {
	...
	protected void bindClusterTrackingService(
	...
	public static final Logger LOG = LoggerFactory
			.getLogger(FilesUploadServlet.class);
	...
	protected void doPost(SlingHttpServletRequest request,
		...
		try {
			...
			ExtendedJSONWriter writer = new ExtendedJSONWriter(response
					.getWriter());
			writer.object();
			writer.key("files");
			writer.array();
			for (Node fileNode : fileNodes) {
				...
				writer.key("id");
				writer.value(fileNode.getProperty(FilesConstants.SAKAI_ID)
						.getString());
				writer.endObject();
			}
			writer.endArray();
			...
			// We send a 200 because SWFUpload has some problems dealing with other status codes.
	...
	private Node createFile(Session session, String store, RequestParameter file)
		...
		Node fileNode = FileUtils
				.saveFile(session, path, id, file, contentType);
		return fileNode;
	}
