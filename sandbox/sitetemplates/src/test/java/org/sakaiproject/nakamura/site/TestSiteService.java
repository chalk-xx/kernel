...
public class TestSiteService extends AbstractEasyMockTest {
  ...
  @Test
  public void testNoDuplicateMembers() throws RepositoryException, SiteException {
    ...
    addPropertyToNode(siteNode, AUTHORIZABLE, new Value[] { new MockValue("group1"),
    ...
    replay();
    AbstractCollection<AuthorizableKey> users = siteService.getMembers(siteNode, 0, 3, null); 
    Iterator<AuthorizableKey> members = users.iterator();
    Set<String> userNames = new HashSet<String>();
    while (members.hasNext()) {
      AuthorizableKey result = members.next();
      assertFalse("Expected users to be unique - already have user: " + result.getAuthorizable().getID(), userNames
          .contains(result.getAuthorizable().getID()));
      userNames.add(result.getAuthorizable().getID());
    }
