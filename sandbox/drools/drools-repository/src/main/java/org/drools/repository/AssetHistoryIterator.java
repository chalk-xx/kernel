...
public class AssetHistoryIterator
    ...
    public AssetHistoryIterator(RulesRepository repo, Node head) {
        ...
        this.repo = repo;
        try {
            VersionManager versionManager = this.head.getSession().getWorkspace().getVersionManager();
            this.versionIterator =  versionManager.getVersionHistory(head.getPath()).getAllVersions();
