...
public class ArchiveItemTest extends TestCase {
    ...
    public void testFindArchivedAssetsByCategory() {
        ...
        List<AssetItem> rules = getRepo().findAssetsByCategory( "testFindRulesByCatArchive", 0, -1  ).assets;
        assertEquals( 1,
                      rules.size() );

        List<AssetItem> rules1 = getRepo().findAssetsByCategory( "testFindRulesByCatArchive",
                                                      true, 0, -1  ).assets;
        assertEquals( 2,
                      rules1.size() );
    }
    ...
    public void testFindRulesByNameArchived() throws Exception {
        ...
        List<AssetItem> list = iteratorToList( repo.findAssetsByName( "findRulesByNameArchived1" ) );
        assertEquals( 1,
                      list.size() );
