...
class TC_Kern1024Test < Test::Unit::TestCase
  ...
  def test_find_all_groups
    ...
    assert_equal(1, managedgroups.size, "Should have one managed group")
    assert_equal(managedgroup.name, managedgroups[0]["sakai:group-id"], "Did not retrieve the managed group")
    ...
    assert_not_nil(groups.find{|e| e["sakai:group-id"] == managedgroup.name}, "Expected group not returned")
    assert_not_nil(groups.find{|e| e["sakai:group-id"] == membergroup.name}, "Expected group not returned")
  end
  ...
  def test_find_matching_groups
    ...
    assert_equal("200", res.code, "My Managed Groups servlet should return successfully")
    managedgroups = JSON.parse(res.body)
    assert_equal(3, managedgroups.size, "Should have three managed groups")
    assert_not_nil(managedgroups.find{|e| e["sakai:group-id"] == managedgroupMatching.name}, "Expected group not returned")
    assert_not_nil(managedgroups.find{|e| e["sakai:group-id"] == managedgroupEmbeddedMatching.name}, "Expected group not returned")
    assert_not_nil(managedgroups.find{|e| e["sakai:group-id"] == managedgroupNotMatching.name}, "Expected group not returned")
    res = @s.execute_get(@s.url_for("/system/me/managedgroups.json?q=#{m}"))
    assert_equal("200", res.code, "My Managed Groups servlet should return successfully")
    managedgroups = JSON.parse(res.body)
    assert_equal(1, managedgroups.size, "Should have one filtered managed group")
    assert_equal(managedgroupMatching.name, managedgroups[0]["sakai:group-id"], "Did not retrieve the managed group")
    res = @s.execute_get(@s.url_for("/system/me/managedgroups.json?q=*#{other}*"))
    assert_equal("200", res.code, "My Managed Groups servlet should return successfully")
    managedgroups = JSON.parse(res.body)
    assert_equal(2, managedgroups.size, "Should have two filtered managed groups from wildcarded search")
    assert_not_nil(managedgroups.find{|e| e["sakai:group-id"] == managedgroupEmbeddedMatching.name}, "Expected group not returned")
    assert_not_nil(managedgroups.find{|e| e["sakai:group-id"] == managedgroupMatching.name}, "Expected group not returned")
    assert_equal(managedgroupMatching.name, managedgroups[0]["sakai:group-id"], "Did not retrieve the managed group")
  end
