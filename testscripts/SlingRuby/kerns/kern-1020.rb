...
class TC_Kern1020Test < Test::Unit::TestCase
  ...
  def test_find_all_groups
    ...
    res = @s.execute_get(@s.url_for("/system/lite/userManager/group/#{managedgroup.name}.tidy.json"))
    @log.info("ManagedGroup #{res.body}")
    res = @s.execute_get(@s.url_for("/system/lite/userManager/group/#{managedgroup.name}-managers.tidy.json"))
    @log.info("ManagedGroup Managers  #{res.body}")
    res = @s.execute_get(@s.url_for("/system/lite/userManager/group/#{membergroup.name}.tidy.json"))
    @log.info("Member Group  #{res.body}")
    res = @s.execute_get(@s.url_for("/system/lite/userManager/group/#{membergroup.name}-managers.tidy.json"))
    @log.info("Member Group Managers  #{res.body}")
    res = @s.execute_get(@s.url_for("/system/lite/userManager/user/#{manager.name}.tidy.json"))
    @log.info("Manager User  #{res.body}")
    res = @s.execute_get(@s.url_for("/system/lite/userManager/user/#{member.name}.tidy.json"))
    @log.info("Member User  #{res.body}")

    res = @s.execute_get(@s.url_for("/system/me.tidy.json"))
    assert_equal("200", res.code, "Me servlet should return successfully")
    me = JSON.parse(res.body)
    groups = me["groups"]
    assert_equal(2, groups.size, "Should have two groups in summary #{managedgroup.name} and #{membergroup.name}  #{res.body}")
